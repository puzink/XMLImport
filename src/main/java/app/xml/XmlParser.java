 ...
 * Xml-файл можно представить в виде дерева, узлами({@link app.xml.Node}) которого являются
 * элементы({@link Element}) Xml-файла. У каждого элемента есть открывающий и закрывающий тэги
 * и поддерево узлов-потомков.
 * Классы реализующие этот интерфейс должны проходить по узлам({@link app.xml.Node}) дерева в глубину.
 * Пример:
 * <table>
 *     <row>
 *         <cell>cellValue</cell>
 ...
 *     <field>fValue</field>
 * </table>
 * В примере выше, сначала будет прочитаны узлы в следующем порядке: table->row->cell->column->field.
 **/
public interface XmlParser extends Closeable {

    /**
     * Проверяет, есть ли не посещенные узлы в xml-файле.
     * Если значение равно false, значит все узлы в дереве пройден и
    ...
    /**
     * Возвращает следующий узел дерева при обходе в глубину.
